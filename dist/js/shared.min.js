...
var _AMFWP = AMFWP,
    longLifeToken = _AMFWP.longLifeToken;
		...
		case 'Initialized':
			...
			if (wp.data) {
				...
				for (var i = 0; i < maps.length; i++) {
					var map = new mapkit.Map(document.getElementById(maps[i].id));
					map.showsMapTypeControl = false;
					map.showsCompass = mapkit.FeatureVisibility.Hidden;
					map.region = new mapkit.CoordinateRegion(new mapkit.Coordinate(parseFloat(maps[i].getAttribute('data-lat')), parseFloat(maps[i].getAttribute('data-long'))), new mapkit.CoordinateSpan(parseFloat(maps[i].getAttribute('data-latd')), parseFloat(maps[i].getAttribute('data-longd'))));
				}
			}
			break;
		case 'Refreshed':
